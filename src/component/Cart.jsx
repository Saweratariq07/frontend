...
import { useState } from "react"; // useState import karna zaroori hai

const Cart = () => {
  const [cart, setCart] = useState([
    { id: 1, name: "Zinger Burger", price: 550, image: "/Image/a.png" }
  ]);
          ...
          cart.map((item) => (
            <div key={item.id} className="border rounded-lg p-4 bg-white shadow-lg w-80 relative group">
              ...
              <div className="absolute top-2 left-2 right-2 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                <button className="bg-white p-2 rounded-full shadow-md" onClick={() => removeFromCart(item.id)}>
